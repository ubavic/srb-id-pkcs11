...
class SmartCard {
  ...
  public:
	...
	static std::vector<uint8_t> BuildAPDU(uint8_t cla, uint8_t ins, uint8_t p1, uint8_t p2, const std::vector<uint8_t>& data, uint32_t ne) {
	...
	static bool ResponseOK(const std::vector<uint8_t>& response) {
		return response.size() >= 2 && response[response.size() - 2] == 0x90 && response[response.size() - 1] == 0x00;
	}

	static bool ValidatePin(const std::string& pin) {
